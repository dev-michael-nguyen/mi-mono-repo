...
const directoryIndexRoutes: Array<string> = ['/offline'];
    ...
    case ServiceWorkerMessageType.DOWNLOAD_MANIFEST:
      ...
      self.clients
        ...
        .then((clients) => {
          ...
          const total =
            manifest.length + event.data.additionalManifestUrls.length;
          let count = 0;

          manifest
            .map((entry) => createCacheKey(entry).url)
            .concat(
              event.data.additionalManifestUrls.map(
                (url) => new URL(url, location.href).href,
              ),
            )
            .forEach((url) => {
              fetchFromNetwork(url).then(() => {
                count++;
                const message = `Downloaded ${count}/${total} files...`;
                const percent = Math.ceil((count * 100) / total);
                logger.log(message, url);
                clients[0].postMessage({
                  type: 'CURRENT_DOWNLOAD_PROGRESS',
                  url,
                  count,
                  total,
                  message,
...
function getCacheKeyForURL(url: string) {
  ...
  if (
    directoryIndex &&
    directoryIndexRoutes.find((route) => urlObject.pathname === route)
  ) {
