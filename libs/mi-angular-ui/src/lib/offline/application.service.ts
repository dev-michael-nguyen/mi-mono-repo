...
export class ApplicationService {
  ...
  constructor(
    ...
    this.hasActivatedSw = this.isSwSupported
  ...
  registerServiceWorker() {
    ...
    this.wb.addEventListener('waiting', () => {
      ...
      this.isUpdateRequested$.pipe(
        ...
        this.wb.messageSW({ type: ServiceWorkerMessageType.DELETE_RUNTIME_CACHE });
    ...
    this.wb.addEventListener('externalwaiting', event => {
      ...
      this.isUpdateRequested$.pipe(
        ...
        event.sw.postMessage({ type: ServiceWorkerMessageType.DELETE_RUNTIME_CACHE });
        dialogRef.componentInstance.isUpdating$.next(true);
        this.isUpdating$.next(true);
  ...
  showUpdateDialog() {
    ...
    dialogRef.componentInstance.isUpdateRequested$.pipe(
      filter((isUpdateRequested) => isUpdateRequested),
      first(),
    ).subscribe(() => {
  ...
  downloadManifest(showDownloadProgressDialog = true) {
    ...
    if (!this.isOnline$.value) {
      return this.generateErrorResponse('Cannot download offline resources because there is no connection.');
    }

    if (this.isSwSupported) {
      this.isManifestDownloaded$.next(false);
      const dialogRef = showDownloadProgressDialog ? this.showDownloadProgressDialog() : null;
      ...
      if (this.hasActivatedSw) {
        ...
        return this.generateErrorResponse('Cannot download offline resources because there is no active SW.');
      }

      return new Observable<IApplicationServiceResponse>(observer => {
        this.isManifestDownloaded$.pipe(
          filter((value) => value),
          first(),
        ).subscribe(_ => {
      ...
      return this.generateErrorResponse('Cannot download offline resources because SW is not suported.');
    }
  }
  ...
  generateErrorResponse(message: string) {
    return new Observable<IApplicationServiceResponse>(observer => {
      observer.next({
        hasError: true,
        responseMessages: [
          { responseMessageType: 3, value: message }
        ]
      });
      observer.complete();
    });
